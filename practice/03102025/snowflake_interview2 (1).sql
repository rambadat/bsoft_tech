...
12.3) Typical DBT flow in order
		...
		(Sowf Table)	 item_cateogory.csv	   stg_suppliers.sql (Table)  			  									 dim_suppliers.sql
						  				       stg_orders.sql  (Table Incr model)	  									 fact_orders.sql
										       ----------------------				  									 fact_sales.sql
										       customers_snapshot.sql (snapshot)      									 pending_orders.sql
...
13.1) What is Jinja?
		...
		More of dynamic code rather than static code
...
14.2) Difference between dbt build and dbt run
        dbt run   : Builds models (SQL files under models/) into your target warehouse (only models)
		dbt build : Runs models + seeds + snapshots + tests in a single workflow. 	   (All)
...
16.3) Star Schema
		...
		The dimension tables surround fact table and store descriptive attributes like customer details, product information, supplier, time/date, or region.